...
Процедура ОтборПриОткрытии()
	...
	ОтборФоновыхЗаданий = Новый ХранилищеЗначения(Новый Структура("Начало", ТекущаяДатаСеанса() - ИнтервалДляОтбора));
...
Процедура ВывестиФоновые()
	...
	Если ТипЗнч(Отбор) = Тип("Структура") И Отбор.Свойство("РегламентноеЗадание", РегламентноеЗаданиеОтбора) Тогда
		...
		ТекОтбор.Удалить("РегламентноеЗадание");
		ОтборПоМетоду = Новый Структура("Метод", РегламентноеЗаданиеОтбора.Метаданные.ИмяМетода);
		ОтборПоКлючу = Новый Структура("Ключ", РегламентноеЗаданиеОтбора.Метаданные.Ключ);
		Если СписокРегламентныхЗаданий.НайтиСтроки(ОтборПоМетоду).Количество() = 1 Тогда
			// по имени метода - если с таким именем метода только один экземпляр
			ТекОтбор.Вставить("ИмяМетода", РегламентноеЗаданиеОтбора.Метаданные.ИмяМетода);
		ИначеЕсли СписокРегламентныхЗаданий.НайтиСтроки(ОтборПоКлючу).Количество() = 1 Тогда
			// по имени ключу - если с таким ключом только один экземпляр
			ТекОтбор.Вставить("Ключ", РегламентноеЗаданиеОтбора.Метаданные.Ключ);
		Иначе
			// по имени фонового задания
			ТекОтбор.Вставить("Наименование", "Запуск вручную: " + РегламентноеЗаданиеОтбора.Метаданные.Синоним);
		КонецЕсли;
		...
		Для Каждого ФоновоеРучное Из ФоновыеРучные Цикл
			Если Фоновые.Найти(ФоновоеРучное) = Неопределено Тогда
