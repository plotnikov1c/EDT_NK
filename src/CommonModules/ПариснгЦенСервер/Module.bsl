	...
	Иначе
		...
		Для Каждого ЭлементDOM Из ЭлементыDOM Цикл
			...
			Если Магазин = Справочники.Магазины.OZON Тогда
				...
				Если СтрДлина(ЭлементDOM.ИмяКласса) = 7 И СтрНайти(ЭлементDOM.ТекстовоеСодержимое, "Код товара") = 0 Тогда
...
Функция ПолучитьСтрокиПоиска(Магазин, СчетчикОбхода)
	...
	ИначеЕсли Магазин = Справочники.Магазины.Буквоед И СчетчикОбхода = 3 Тогда
	...
	ИначеЕсли Магазин = Справочники.Магазины.AliExpress И СчетчикОбхода = 1 Тогда
		
		ИмяЭлемента = "div";
		ИмяКласса   = "snow-price_SnowPrice__mainS__18x8np";
		
	ИначеЕсли Магазин = Справочники.Магазины.ЯндексМаркет И СчетчикОбхода = 1 Тогда
		
		ИмяЭлемента = "div";
		ИмяКласса   = "_3NaXx _3kWlK";
